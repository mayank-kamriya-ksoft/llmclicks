...
const CellIcon = ({ val }: { val: boolean | string }) => {
  ...
  if (val === "partial") return <Minus className="h-5 w-5 text-muted-foreground mx-auto" />;
    ...
    <section className="section-padding pt-24 md:pt-32">
      <div className="container mx-auto max-w-5xl">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-16">
          <h1 className="font-display text-4xl md:text-5xl font-bold mb-4">How We <span className="gradient-text">Compare</span></h1>
          <p className="text-lg text-muted-foreground">See why LLMClicks is the best choice for AI visibility.</p>
        </motion.div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                ...
                {tools.map((t) => (
                  <th key={t.name} className={`py-4 px-4 font-display font-semibold text-center ${t.name === "LLMClicks" ? "text-primary" : "text-muted-foreground"}`}>
                    {t.name}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {features.map((feature, fi) => (
                <tr key={feature} className="border-b border-border/50">
                  ...
                  {tools.map((t) => (
                    <td key={t.name} className="py-3 px-4"><CellIcon val={t.values[fi]} /></td>
                  ))}
                </tr>
              ))}
            </tbody>
